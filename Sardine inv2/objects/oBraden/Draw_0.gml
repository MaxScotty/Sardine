...
if posSubMenu == 0
{
	for (var i = 0; i < array_length(braden_items); i++)
	{
		...
		draw_text_color(room_width/2 - room_width/5 + 48 + ((i == array_length(braden_items)-1) ? (48) : (0)), (i != array_length(braden_items)-1) ? (room_height/2 - room_height*0.4+48+32*i) : (room_height/2+96), /* добавляем "-", если i не на кнопке выхода */ $"{(i != array_length(braden_items)-1) ? ("-") : ("")}{item.Name}", _c, _c, _c,_c, 1);	
	}
}
else
{
	for (var i = 0; i < array_length(player_items); i++)
	{
		...
		draw_text_color(room_width/2 - room_width/5 + 48 + ((i == array_length(player_items)-1) ? (48) : (0)), (i != array_length(player_items)-1) ? (room_height/2 - room_height*0.4+48+32*i) : (room_height/2+96), /* добавляем "-", если i не на кнопке выхода */ $"{(i != array_length(player_items)-1) ? ("-") : ("")}{item.Name}{_number}", _c, _c, _c,_c, 1);	
	}
}
